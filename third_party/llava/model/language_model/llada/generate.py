...
def recursive_set_attr(obj, fn):
    """
    递归地对任意嵌套结构（tensor/tuple/list/dict/set）中的所有 tensor 执行 fn(tensor)。
    返回结构不变的新对象。
    """
    if isinstance(obj, torch.Tensor):
        ...
        return {k: recursive_set_attr(v, fn) for k, v in obj.items()}
    elif isinstance(obj, (list, tuple, set)):
        t = type(obj)
        return t(recursive_set_attr(x, fn) for x in obj)
    else:
        return obj

# 用法示例：递归 retain_grad
def recursive_retain_grad(obj):
    def _retain(t):
        t.retain_grad()
    return recursive_set_attr(obj, _retain)
...
def generate(model, prompt=None, steps=None, max_new_tokens=128, block_length=128, temperature=0.,
    ...
    if prefix_lm:
        ...
        past_key_values = recursive_retain_grad(past_key_values)
    else:
        x = torch.full((1, prompt.shape[1] + gen_length), mask_id, dtype=torch.long).to(model.device)
        # 非覆盖写法
        x = torch.cat([prompt.clone(), x[:, prompt.shape[1]:]], dim=1)
    ...
    if draft_tokens is not None:
        ...
        x = torch.cat([
            x[:, :prompt.shape[1]],
            draft_tokens.clone(),
            x[:, prompt.shape[1]+draft_tokens.shape[1]:]
        ], dim=1)
    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            if cfg_scale > 0.:
                ...
                un_x = x.clone()
                # 非覆盖写法
                un_x = torch.where(prompt_index, mask_id, un_x)
            ...
            cam_logits, x0 = torch.max(logits_with_noise, dim=-1) # b, l
            all_cam_logits.append(cam_logits)
            ...
            # 非覆盖写法
            mask = torch.zeros_like(x0_p, dtype=torch.bool)
            mask[:, prompt.shape[1] + (num_block + 1) * block_length:] = True
            x0_p = torch.where(mask, torch.tensor(-np.inf, device=x0_p.device, dtype=x0_p.dtype), x0_p)

            x0 = torch.where(mask_index, x0, x)
            confidence = torch.where(mask_index, x0_p, torch.tensor(-np.inf, device=x0_p.device, dtype=x0_p.dtype))
            ...
            for j in range(confidence.shape[0]):
                try:
                    ...
                    breakpoint()
                # 非覆盖写法
                temp = transfer_index[j].clone()
                temp[select_index] = True
                transfer_index = transfer_index.clone()
                transfer_index[j] = temp
            # 非覆盖写法
    ...
    if verbose:
        return x, history, all_cam_logits
    return x, all_cam_logits

...
class LlavaLladaForMaskedDiffusion(LLaDAModelLM,LlavaMetaForCausalLM):
    ...
    def _retain_grad_hook(self, module, input, output):
        # 对所有输出张量调用retain_grad，保证梯度可用
        # if isinstance(output, torch.Tensor):
        #     if output.requires_grad and not output.is_leaf:
        #         output.retain_grad()
        # elif isinstance(output, (tuple, list)):
        #     for o in output:
        #         if isinstance(o, torch.Tensor) and o.requires_grad and not o.is_leaf:
        #             o.retain_grad()
        if isinstance(output, torch.Tensor):
            if not output.requires_grad:
                output.requires_grad = True
            if not output.is_leaf:
                output.retain_grad()
        elif isinstance(output, (tuple, list)):
            for o in output:
    ...
    def register_retain_grad_hooks(self):
        """
        给所有子模块注册forward hook，自动对正向传播的所有输出张量retain_grad。
        """
        for module in self.modules():
            module.register_forward_hook(self._retain_grad_hook)
    ...
    def __init__(self, config: LLaDAConfig, model: Optional[LLaDAModel] = None, init_params: bool = False,vision_kwargs=None,prefix_lm=False,**kwargs):
        ...
        self.post_init() # TODO
        self.register_retain_grad_hooks()
    ...
    def generate(
        ...
        **kwargs,
        ...
        if images is not None:
            (inputs, position_ids, attention_mask, _, inputs_embeds, _, info4cam) = self.prepare_inputs_labels_for_multimodal(inputs, position_ids, attention_mask, None, None, images, modalities, image_sizes=image_sizes)
        else:
            # breakpoint()
        ...
        return llada_generate(self.get_model(),inputs_embeds=inputs_embeds,position_ids=position_ids,attention_mask=attention_mask,**kwargs), info4cam
